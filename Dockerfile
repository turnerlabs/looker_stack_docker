...
RUN echo "[INFO]::[download]::[looker]" \
    ...
    -d '{"lic": "'${LOOKER_LICENSE_KEY}'", "email": "'${LOOKER_LICENSE_EMAIL}'", "latest": "specific", "specific": "looker-latest.jar"}' \
    https://apidownload.looker.com/download | jq '.url' | xargs curl -o /home/looker/looker/looker.jar

RUN echo "[INFO]::[download]::[looker dependencies]" \
    && set -ex \
    && curl -X POST -H 'Content-Type: application/json' \
    -d '{"lic": "'${LOOKER_LICENSE_KEY}'", "email": "'${LOOKER_LICENSE_EMAIL}'", "latest": "specific", "specific": "looker-latest.jar"}' \
    https://apidownload.looker.com/download | jq '.depUrl' | xargs curl -o /home/looker/looker/looker-dependencies.jar
